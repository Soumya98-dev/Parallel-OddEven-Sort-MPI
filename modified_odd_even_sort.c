...
// Function to compare and exchange elements during sorting
void CompareExchange(int *local_array, int nlocal, int partner, int myrank, MPI_Comm comm) {
    ...
    // Merge the two arrays based on rank (even or odd phase)
...
int main(int argc, char *argv[]) {
    int n = 128;  // Default number of elements
    ...
    if (myrank == 0) {
        ...
        for (int i = 0; i < n; i++) {
            array[i] = rand() % 129; // Random integers in the range [0, 128]
        }
    ...
    for (int phase = 0; phase < nprocs; phase++) {
        if (phase % 2 == 0) {
            ...
            if (myrank % 2 == 0 && myrank < nprocs - 1) {
                CompareExchange(local_array, nlocal, myrank + 1, myrank, MPI_COMM_WORLD);
            } else if (myrank % 2 == 1) {
                CompareExchange(local_array, nlocal, myrank - 1, myrank, MPI_COMM_WORLD);
            }
        } else {
    ...
    if (myrank == 0) {
        ...
        if (time_file) {
            fprintf(time_file, "[Modified Odd-Even Sort] Array size: %d, Processes: %d, Time: %f seconds\n", n, nprocs, execution_time);
            fclose(time_file);
        }
        ...
        if (result_file) {
            fprintf(result_file, "[Modified Odd-Even Sort] Array size: %d\n", n);
            for (int i = 0; i < n; i++) {
